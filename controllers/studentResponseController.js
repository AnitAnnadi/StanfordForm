...
const getStudentResponses = async (req, res) => {
  try {
      ...
      for (const [year, isChecked] of Object.entries(checkedYears)) {
        if (isChecked === "true") {
          ...
          if (year === "2023-2024") {
            ...
            startDate = new Date("2025-01-04T00:00:00Z");
    ...
    res
      ...
      .json({ teacherName: teacher?.name, studentResponses });
  } catch (error) {
    console.log(error);
    res
...
const getHealthyFutures = async (req, res) => {
  ...
  const responsesByCannabis = responses.filter(
    (response) => response.formType === "Healthy Futures: Cannabis"
  );
  const responsesByTobacco = responses.filter(
    (response) =>
      response.formType === "Healthy Futures: Tobacco/Nicotine/Vaping"
  );
