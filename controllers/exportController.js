...
import { tobacco, postTobacco, cannabis, postCannabis, safety } from "../questions.js";
let exportData = []
...
const getExport = async (req, res) => {
  ...
  try {
    ...
    const map = await Promise.all (studentResponses.map(async (studentResponse) => {
      let obj = {teacher:teacher.name, county:school.county, district:school.district, city:school.city, }      
      obj.when=studentResponse.when
      obj.grade = studentResponse.grade
      obj.period = studentResponse.period === undefined ? 'n/a' : studentResponse.period;
      obj["form type"] =studentResponse.formType


      let questions = await Question.find({ StudentResponse: studentResponse._id });
      ...
      else if (studentResponse.formType==='Smart Talk: Cannabis Prevention & Education Awareness'){
        if (studentResponse.when==='before'){
    ...
    res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ error: "Failed to export exportData." });
  }
};
