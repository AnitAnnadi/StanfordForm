  ...
  return (
    <Wrapper>
      <header>
        <div className='info'>
          <h3>{school.school}</h3>
          <h4>{school.city}, {school.state}</h4>
        </div>
      </header>
      <div className='content'>
        <div className='content-center'>
            <ResponseGroupInfo icon={<FaChalkboardTeacher />} text={teacher.name} />
            <ResponseGroupInfo icon={<AiOutlineNumber />} text={formCode} />
            <ResponseGroupInfo icon={<TbListNumbers />} text=
            {responseType?.period ? 'Period ' + responseType.period:
              'No specified period'}
            />
            <ResponseGroupInfo icon={<TbNumbers />} text={'Grade ' + responseType.grade} />
            <ResponseGroupInfo icon={<AiOutlineForm />} text={responseType.formType} />
            <ResponseGroupInfo icon={<FaRegCalendarAlt />} text={responseType.when} />
        </div>
      </div>
      {/*<header>*/}
      {/*  <h3>{`Form metrics`}</h3>*/}
      {/*  <h4>{`${school.school}`}</h4>*/}
      {/*  <h4>{`${school.city}, ${school.state}`}</h4>*/}
      {/*  <h4>{`${teacher.name}'s class`}</h4>*/}
      {/*  <h5>{`Form code: ${formCode}`}</h5>*/}
      {/*  <h5>{`Period: ${responseType.period ? responseType.period: "unspecified"}`}</h5>*/}
      {/*  <h5>{`Grade: ${responseType.grade}`}</h5>*/}
      {/*  <h5>{`Form type: ${responseType.formType}`}</h5>*/}
      {/*  <h5>{`when: ${responseType.when}`}</h5>*/}
      {/*</header>*/}
      ...
      <div className='content'>
        <div className='content-center'>
          {Object.keys(questionsToAnswers).map((question, index) => (
            <div key={index}>
              <h5 style={{padding: "1rem 1.5rem"}}>{question}</h5>
                      ...
                      datasets: [
                        {
                          ...
                          backgroundColor: Object.keys(questionsToAnswers[question]).map(() => dynamicColors()),
                          borderWidth: 1
                        }
                      ]
                    }
      ...
      <footer>
        <button className='btn btn-block' onClick={() => navigate(-1)}>Go back</button>
      </footer>
    </Wrapper>
  );
};
